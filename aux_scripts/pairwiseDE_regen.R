...
dataset = readRDS("results/cirrhosis/sub_allcells_css.RDS")

# pairwise DE
avg_exp = list()
mk_list = list()
for(g in c("subpops", "anno_cond")[1]){
...
# save results
saveRDS(mk_list, file = "results/cirrhosis/regen_markers.RDS")
saveRDS(avg_exp, file = "results/cirrhosis/regen_avgExp.RDS")