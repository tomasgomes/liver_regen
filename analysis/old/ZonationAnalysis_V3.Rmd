...
rmeta = cbind(hep_cells$regenerating@meta.data,
              hep_cells$regenerating@reductions$umap@cell.embeddings)

save(hscaled, escaled, rscaled, hmeta, emeta, rmeta, file = "to_send/hep_scaled_meta.RData")
```
...
df_dc = cbind(end_cells$healthy@meta.data, 
              data.frame("DC1" = dpt@dm$DC1, "DC2" = dpt@dm$DC2, "DPT" = dpt$DPT363),
...
for(n in names(traj_list)){
  ...
  write.csv(end_cells[[n]]@meta.data[,c("unique_name", "Condition", 
                                        "zonation_pt", "zonation_int")],
            file = paste0("results/zonation_cond/end_zonation_", n, "_rank.csv"),
            col.names = T, row.names = T, quote = F)
}

saveRDS(end_cells, file = "results/zonation_cond/end_cells_zonation_rank.RDS")
```
...
all_end_cells = Reduce(merge, end_cells)
```
...
exp_g = list("healthy" = Matrix::rowSums(end_cells[["healthy"]]@assays$SCT@data>0)>=30,
             "embolised" = Matrix::rowSums(end_cells[["embolised"]]@assays$SCT@data>0)>=30,
             "regenerating" = Matrix::rowSums(end_cells[["regenerating"]]@assays$SCT@data>0)>=30)

hvg = hvg[hvg %in% rownames(end_cells[["healthy"]]@assays$SCT@data)[exp_g$healthy] &
            hvg %in% rownames(end_cells[["embolised"]]@assays$SCT@data)[exp_g$embolised] &
            hvg %in% rownames(end_cells[["regenerating"]]@assays$SCT@data)[exp_g$regenerating]]

# remove HVG that are markers for other cell types
hvg = hvg[!(hvg %in% unique(unlist(top_genes$cell_type[!grepl("LSEC ", names(top_genes$cell_type))])))]
```
...
saveRDS(end_fits_qval, file = "results/zonation_cond/end_fits_qval.RDS")
```
...
```{r}
# genes that vary in at least one condition
var_pval = cbind(end_fits_qval$healthy$fdr<=0.05,
                 end_fits_qval$embolised$fdr<=0.05,
                 end_fits_qval$regenerating$fdr<=0.05)
colnames(var_pval) = c("healthy", "embolised", "regenerating")

# some variability in expression
end_sig_fits = lapply(end_fits_qval, function(x) x$fits)
end_sig_fits$healthy[end_sig_fits$healthy<0] = 0
end_sig_fits$embolised[end_sig_fits$embolised<0] = 0
end_sig_fits$regenerating[end_sig_fits$regenerating<0] = 0
gvar = as.data.frame(lapply(end_sig_fits, function(x) apply(x, 2, function(y) diff(range(y))>=0.05)))

end_sc_fits = lapply(end_sig_fits, function(x) t(apply(x, 2, 
                     function(x) if(all(x==x[1])){rep(0, length(x))} else{minmax_scale(x)})))

# get correlations
cor_he = unlist(lapply(rownames(end_sc_fits$healthy), 
                       function(x) cor(end_sc_fits$healthy[x,],end_sc_fits$embolised[x,], method = "sp")))
cor_hr = unlist(lapply(rownames(end_sc_fits$healthy), 
                       function(x) cor(end_sc_fits$healthy[x,],end_sc_fits$regenerating[x,], method = "sp")))
names(cor_he) = names(cor_hr) = rownames(end_sc_fits$healthy)
...
for(n in names(notsim)){
  ...
  end_both_norm = Reduce(rbind, lapply(genes_use, 
                                       function(x) minmax_scale(c(end_sig_fits$healthy[,x],
                                                                  end_sig_fits[[n]][,x]))))
  rownames(end_both_norm) = genes_use
  
  hcl = hclust(as.dist(1-cor(t(end_both_norm[,1:100]-end_both_norm[,101:200]), 
                              method = "sp")), method = "ward.D")
  cls = cutree(hcl, 5)
  
  plot_df = reshape2::melt(t(end_both_norm[,1:100]-end_both_norm[,101:200]))
  plot_df = merge(plot_df, data.frame("cls" = cls), by.x = 2, by.y = 0)
  plot_df$cond = "difference"
  ...
  plot_df_l = list()
  for(n2 in names(condrange)){
    plot_df2 = reshape2::melt(t(end_both_norm[,condrange[[n2]]]))
    plot_df2 = merge(plot_df2, data.frame("cls" = cls), by.x = 2, by.y = 0)
    plot_df2$cond = n2
    plot_df_l[[n2]] = plot_df2
  }
  ...
  end_res_list[[n]] = list("plot" = plt, "plt_all" = plt_all, "go" = got_list,
                           "go_cl" = got_cl_list, "df" = plot_df2, "dat" = all_dat)
}

eg_all = clusterProfiler::bitr(rownames(end_res_list$embolised$dat), fromType="SYMBOL", 
                               toType="ENTREZID", OrgDb="org.Hs.eg.db")
eg_cor = rownames(end_res_list$embolised$dat)[((end_res_list$embolised$dat[,1] &
                                                 end_res_list$embolised$dat[,2]) | 
                                                (end_res_list$embolised$dat[,3] &
                                                   end_res_list$embolised$dat[,4])) &
                                                end_res_list$embolised$dat[,5]>=0.3]
eg = clusterProfiler::bitr(eg_cor, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end_he_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_all$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end_he_got_top = getTopTerms(end_he_got@result[end_he_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end_res_list$embolised$go$pos = end_he_got@result[end_he_got@result$p.adjust<=0.05,]
end_res_list$embolised$go_cl$pos = end_he_got_top
gemb = rownames(end_res_list$embolised$dat)[((end_res_list$embolised$dat[,1] &
                                                end_res_list$embolised$dat[,2]) | 
                                              (end_res_list$embolised$dat[,3] &
                                                 end_res_list$embolised$dat[,4]))]
eg_emb = clusterProfiler::bitr(gemb, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end_he_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_emb$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end_he_got_top = getTopTerms(end_he_got@result[end_he_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end_res_list$embolised$go$poscor = end_he_got@result[end_he_got@result$p.adjust<=0.05,]
end_res_list$embolised$go_cl$poscor = end_he_got_top

eg_cor = rownames(end_res_list$regenerating$dat)[((end_res_list$regenerating$dat[,1] &
                                                 end_res_list$regenerating$dat[,2]) | 
                                                (end_res_list$regenerating$dat[,3] &
                                                   end_res_list$regenerating$dat[,4])) &
                                                end_res_list$regenerating$dat[,5]>=0.3]
eg = clusterProfiler::bitr(eg_cor, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end_hr_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_all$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end_hr_got_top = getTopTerms(end_hr_got@result[end_hr_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end_res_list$regenerating$go$pos = end_hr_got@result[end_hr_got@result$p.adjust<=0.05,]
end_res_list$regenerating$go_cl$pos = end_hr_got_top
greg = rownames(end_res_list$regenerating$dat)[((end_res_list$regenerating$dat[,1] &
                                                end_res_list$regenerating$dat[,2]) | 
                                              (end_res_list$regenerating$dat[,3] &
                                                 end_res_list$regenerating$dat[,4]))]
eg_reg = clusterProfiler::bitr(greg, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end_hr_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_reg$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end_hr_got_top = getTopTerms(end_hr_got@result[end_hr_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end_res_list$regenerating$go$poscor = end_hr_got@result[end_hr_got@result$p.adjust<=0.05,]
end_res_list$regenerating$go_cl$poscor = end_hr_got_top

saveRDS(end_res_list, file = "results/zonation_cond/end_got_clustering_list.RDS")
```

Save tables

```{r}
for(cl in names(end_res_list$embolised$go)){
  write.csv(end_res_list$embolised$go[[cl]], 
            file = paste0("to_send/gozonation/end/end_go_allsig_cl", cl, "_vEmb.csv"),
            col.names = T, row.names = T, quote = F)
  write.csv(end_res_list$embolised$go_cl[[cl]], 
            file = paste0("to_send/gozonation/end/end_go_clustSummary_cl", cl, "_vEmb.csv"),
            col.names = T, row.names = T, quote = F)
}
write.csv(unique(end_res_list$embolised$df[,c(1,4)]), 
          file = paste0("to_send/gozonation/end/end_clusters_vEmb.csv"),
          col.names = T, row.names = T, quote = F)
for(cl in names(end_res_list$regenerating$go)){
  write.csv(end_res_list$regenerating$go[[cl]], 
            file = paste0("to_send/gozonation/end/end_go_allsig_cl", cl, "_vReg.csv"),
            col.names = T, row.names = T, quote = F)
  write.csv(end_res_list$regenerating$go_cl[[cl]], 
            file = paste0("to_send/gozonation/end/end_go_clustSummary_cl", cl, "_vReg.csv"),
            col.names = T, row.names = T, quote = F)
}
write.csv(unique(end_res_list$regenerating$df[,c(1,4)]), 
          file = paste0("to_send/gozonation/end/end_clusters_vReg.csv"),
          col.names = T, row.names = T, quote = F)

pdf("to_send/gozonation/end/end_allGenes_vEmb.pdf", height = 4, width = 7)
print(end_res_list$embolised$plt_all)
dev.off()
pdf("to_send/gozonation/end/end_meanCI_vEmb.pdf", height = 2, width = 5)
print(end_res_list$embolised$plot)
dev.off()

pdf("to_send/gozonation/end/end_allGenes_vReg.pdf", height = 4, width = 7)
print(end_res_list$regenerating$plt_all)
dev.off()
pdf("to_send/gozonation/end/end_meanCI_vReg.pdf", height = 2, width = 5)
print(end_res_list$regenerating$plot)
dev.off()
```
...
```{r}
g = "CLEC1A"
plt_df = data.frame("exp" = c(end_sig_fits$healthy[,g], end_sig_fits$embolised[,g]),
                    "cond" = rep(c("healthy", "embolised"), each = 100),
                    "t" = rep(1:100, 2))
ggplot(plt_df, aes(x = t, y = exp, group = cond, col = cond))+
  geom_line()+
  labs(x = "zonation", y = "Expression", title = g)+
  theme_bw()+
  theme(aspect.ratio = 1/1.5)
...
g = "ITGAV"
plt_df = data.frame("exp" = c(end_sig_fits$healthy[,g], end_sig_fits$regenerating[,g]),
                    "cond" = rep(c("healthy", "regenerating"), each = 100),
                    "t" = rep(1:100, 2))
ggplot(plt_df, aes(x = t, y = exp, group = cond, col = cond))+
  geom_line()+
...
plot(end_sig_fits$healthy[,g]-end_sig_fits$regenerating[,g])
plot(end_sc_fits$healthy[g,]-end_sc_fits$regenerating[g,])
xxx = minmax_scale(c(end_sig_fits$healthy[,g], end_sig_fits$regenerating[,g]))
plot(xxx[1:100]-xxx[101:200])
```

Plot GO Terms (general)

```{r}
plot_df = end_res_list$embolised$go_cl$all
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
...
plot_df = end_res_list$embolised$go_cl$pos
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
...
plot_df = end_res_list$regenerating$go_cl$all
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
  scale_x_continuous(expand = c(0,0), lim = c(0,2))+
  labs(title = "GO Terms - Regenerating", subtitle = "cor < 0.3")+
  theme_bw()+
  theme(axis.text = element_text(colour = "black"),
        axis.text.y = element_text(vjust = 0.6),
...
plot_df = end_res_list$regenerating$go_cl$pos
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
  scale_x_continuous(expand = c(0,0), lim = c(0,11))+
  labs(title = "GO Terms - Regenerating", subtitle = "cor >= 0.3")+
  theme_bw()+
  theme(axis.text = element_text(colour = "black"),
        axis.text.y = element_text(vjust = 0.6),
...
for(cond in names(end_cells)){
  ...
  sub_dat = end_cells[[cond]][,end_cells[[cond]]$zonation_pt<=0.8]
  # Fit GAM for each gene using pseudotime as independent variable.
  cond_fit_list[[cond]] = mclapply(hvg, fittingFunc, mc.cores = numCores)
}
end80_fits_qval = list()
for(cond in names(cond_fit_list)){
  fits_df = Reduce(cbind, lapply(cond_fit_list[[cond]], function(x) x$fits))
  ...
  end80_fits_qval[[cond]] = list("fits" = fits_df, "pval" = pval_df,
                               "fdr" = apply(pval_df, 2, function(x) p.adjust(x, method = "fdr")))
}
end80_time <- Sys.time()
end80_time - start_time

saveRDS(end80_fits_qval, file = "results/zonation_cond/end80_fits_qval.RDS")
```
...
```{r}
# genes that vary in at least one condition
var_pval = cbind(end80_fits_qval$healthy$fdr<=0.05,
                 end80_fits_qval$embolised$fdr<=0.05,
                 end80_fits_qval$regenerating$fdr<=0.05)
colnames(var_pval) = c("healthy", "embolised", "regenerating")

# some variability in expression
end80_sig_fits = lapply(end80_fits_qval, function(x) x$fits)
end80_sig_fits$healthy[end80_sig_fits$healthy<0] = 0
end80_sig_fits$embolised[end80_sig_fits$embolised<0] = 0
end80_sig_fits$regenerating[end80_sig_fits$regenerating<0] = 0
gvar = as.data.frame(lapply(end80_sig_fits, function(x) apply(x, 2, function(y) diff(range(y))>=0.05)))

end80_sc_fits = lapply(end80_sig_fits, function(x) t(apply(x, 2, 
                     function(x) if(all(x==x[1])){rep(0, length(x))} else{minmax_scale(x)})))

# get correlations
cor_he = unlist(lapply(rownames(end80_sc_fits$healthy), 
                       function(x) cor(end80_sc_fits$healthy[x,],end80_sc_fits$embolised[x,], method = "sp")))
cor_hr = unlist(lapply(rownames(end80_sc_fits$healthy), 
                       function(x) cor(end80_sc_fits$healthy[x,],end80_sc_fits$regenerating[x,], method = "sp")))
names(cor_he) = names(cor_hr) = rownames(end80_sc_fits$healthy)
...
for(n in names(notsim)){
  ...
  end80_both_norm = Reduce(rbind, lapply(genes_use, 
                                       function(x) minmax_scale(c(end80_sig_fits$healthy[,x],
                                                                  end80_sig_fits[[n]][,x]))))
  rownames(end80_both_norm) = genes_use
  
  hcl = hclust(as.dist(1-cor(t(end80_both_norm[,1:100]-end80_both_norm[,101:200]), 
                              method = "sp")), method = "ward.D")
  cls = cutree(hcl, 5)
  
  plot_df = reshape2::melt(t(end80_both_norm[,1:100]-end80_both_norm[,101:200]))
  plot_df = merge(plot_df, data.frame("cls" = cls), by.x = 2, by.y = 0)
  plot_df$cond = "difference"
  ...
  plot_df_l = list()
  for(n2 in names(condrange)){
    plot_df2 = reshape2::melt(t(end80_both_norm[,condrange[[n2]]]))
    plot_df2 = merge(plot_df2, data.frame("cls" = cls), by.x = 2, by.y = 0)
    plot_df2$cond = n2
    plot_df_l[[n2]] = plot_df2
  }
  ...
  end80_res_list[[n]] = list("plot" = plt, "plt_all" = plt_all, "go" = got_list,
                           "go_cl" = got_cl_list, "df" = plot_df2, "dat" = all_dat)
}

eg_all = clusterProfiler::bitr(rownames(end80_res_list$embolised$dat), fromType="SYMBOL", 
                               toType="ENTREZID", OrgDb="org.Hs.eg.db")
eg_cor = rownames(end80_res_list$embolised$dat)[((end80_res_list$embolised$dat[,1] &
                                                 end80_res_list$embolised$dat[,2]) | 
                                                (end80_res_list$embolised$dat[,3] &
                                                   end80_res_list$embolised$dat[,4])) &
                                                end80_res_list$embolised$dat[,5]>=0.3]
eg = clusterProfiler::bitr(eg_cor, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end80_he_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_all$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end80_he_got_top = getTopTerms(end80_he_got@result[end80_he_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end80_res_list$embolised$go$pos = end80_he_got@result[end80_he_got@result$p.adjust<=0.05,]
end80_res_list$embolised$go_cl$pos = end80_he_got_top
gemb = rownames(end80_res_list$embolised$dat)[((end80_res_list$embolised$dat[,1] &
                                                end80_res_list$embolised$dat[,2]) | 
                                              (end80_res_list$embolised$dat[,3] &
                                                 end80_res_list$embolised$dat[,4]))]
eg_emb = clusterProfiler::bitr(gemb, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end80_he_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_emb$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end80_he_got_top = getTopTerms(end80_he_got@result[end80_he_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end80_res_list$embolised$go$poscor = end80_he_got@result[end80_he_got@result$p.adjust<=0.05,]
end80_res_list$embolised$go_cl$poscor = end80_he_got_top

eg_cor = rownames(end80_res_list$regenerating$dat)[((end80_res_list$regenerating$dat[,1] &
                                                 end80_res_list$regenerating$dat[,2]) | 
                                                (end80_res_list$regenerating$dat[,3] &
                                                   end80_res_list$regenerating$dat[,4])) &
                                                end80_res_list$regenerating$dat[,5]>=0.3]
eg = clusterProfiler::bitr(eg_cor, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end80_hr_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_all$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end80_hr_got_top = getTopTerms(end80_hr_got@result[end80_hr_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end80_res_list$regenerating$go$pos = end80_hr_got@result[end80_hr_got@result$p.adjust<=0.05,]
end80_res_list$regenerating$go_cl$pos = end80_hr_got_top
greg = rownames(end80_res_list$regenerating$dat)[((end80_res_list$regenerating$dat[,1] &
                                                end80_res_list$regenerating$dat[,2]) | 
                                              (end80_res_list$regenerating$dat[,3] &
                                                 end80_res_list$regenerating$dat[,4]))]
eg_reg = clusterProfiler::bitr(greg, fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
end80_hr_got = clusterProfiler::enrichGO(gene = eg$ENTREZID, universe = eg_reg$ENTREZID, 
                                      OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                      qvalueCutoff  = 0.05, 
                                      pAdjustMethod = "BH", readable = T)
end80_hr_got_top = getTopTerms(end80_hr_got@result[end80_hr_got@result$p.adjust<=0.05,], ncl = 10, 
                             nt = 1, topt = 1000)
end80_res_list$regenerating$go$poscor = end80_hr_got@result[end80_hr_got@result$p.adjust<=0.05,]
end80_res_list$regenerating$go_cl$poscor = end80_hr_got_top

saveRDS(end80_res_list, file = "results/zonation_cond/end80_got_clustering_list.RDS")
```

Save tables

```{r}
for(cl in names(end80_res_list$embolised$go)){
  write.csv(end80_res_list$embolised$go[[cl]], 
            file = paste0("to_send/gozonation/end80/end80_go_allsig_cl", cl, "_vEmb.csv"),
            col.names = T, row.names = T, quote = F)
  write.csv(end80_res_list$embolised$go_cl[[cl]], 
            file = paste0("to_send/gozonation/end80/end80_go_clustSummary_cl", cl, "_vEmb.csv"),
            col.names = T, row.names = T, quote = F)
}
write.csv(unique(end80_res_list$embolised$df[,c(1,4)]), 
          file = paste0("to_send/gozonation/end80/end80_clusters_vEmb.csv"),
          col.names = T, row.names = T, quote = F)
for(cl in names(end80_res_list$regenerating$go)){
  write.csv(end80_res_list$regenerating$go[[cl]], 
            file = paste0("to_send/gozonation/end80/end80_go_allsig_cl", cl, "_vReg.csv"),
            col.names = T, row.names = T, quote = F)
  write.csv(end80_res_list$regenerating$go_cl[[cl]], 
            file = paste0("to_send/gozonation/end80/end80_go_clustSummary_cl", cl, "_vReg.csv"),
            col.names = T, row.names = T, quote = F)
}
write.csv(unique(end80_res_list$regenerating$df[,c(1,4)]), 
          file = paste0("to_send/gozonation/end80/end80_clusters_vReg.csv"),
          col.names = T, row.names = T, quote = F)

pdf("to_send/gozonation/end80/end80_allGenes_vEmb.pdf", height = 4, width = 7)
print(end80_res_list$embolised$plt_all)
dev.off()
pdf("to_send/gozonation/end80/end80_meanCI_vEmb.pdf", height = 2, width = 5)
print(end80_res_list$embolised$plot)
dev.off()

pdf("to_send/gozonation/end80/end80_allGenes_vReg.pdf", height = 4, width = 7)
print(end80_res_list$regenerating$plt_all)
dev.off()
pdf("to_send/gozonation/end80/end80_meanCI_vReg.pdf", height = 2, width = 5)
print(end80_res_list$regenerating$plot)
dev.off()
```
...
```{r}
g = "CLEC1A"
plt_df = data.frame("exp" = c(end80_sig_fits$healthy[,g], end80_sig_fits$embolised[,g]),
                    "cond" = rep(c("healthy", "embolised"), each = 100),
                    "t" = rep(1:100, 2))
ggplot(plt_df, aes(x = t, y = exp, group = cond, col = cond))+
  geom_line()+
...
g = "ITGAV"
plt_df = data.frame("exp" = c(end80_sig_fits$healthy[,g], end80_sig_fits$regenerating[,g]),
                    "cond" = rep(c("healthy", "regenerating"), each = 100),
                    "t" = rep(1:100, 2))
ggplot(plt_df, aes(x = t, y = exp, group = cond, col = cond))+
  geom_line()+
...
plot(end80_sig_fits$healthy[,g]-end80_sig_fits$regenerating[,g])
plot(end80_sc_fits$healthy[g,]-end80_sc_fits$regenerating[g,])
xxx = minmax_scale(c(end80_sig_fits$healthy[,g], end80_sig_fits$regenerating[,g]))
plot(xxx[1:100]-xxx[101:200])
```
...
```{r}
plot_df = end80_res_list$embolised$go_cl$all
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
...
plot_df = end80_res_list$embolised$go_cl$pos
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
...
plot_df = end80_res_list$regenerating$go_cl$all
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
...
plot_df = end80_res_list$regenerating$go_cl$pos
plot_df$Description = breakStr(plot_df$Description, 30)
plot_df$Description = factor(plot_df$Description, levels = rev(plot_df$Description))
ggplot(plot_df, aes(x = -log10(qvalue), y = Description))+
  geom_col()+
...
# OLD CODE
Find varying genes - using the first 75% of cells along the ranked pseudotime

```{r}
## get varying genes for each condition
numCores = 12 #larger values resulted in problems
start_time <- Sys.time()
cond_fit75_list = list()
for(cond in names(hep_cells)){
  print(cond)
  
  sub_dat = hep_cells[[cond]][,hep_cells[[cond]]$zonation_pt<0.75]
  # Fit GAM for each gene using pseudotime as independent variable.
  cond_fit75_list[[cond]] = mclapply(hvg, fittingFunc, mc.cores = numCores)
}
hep75_fits_qval = list()
for(cond in names(cond_fit75_list)){
  fits_df = Reduce(cbind, lapply(cond_fit75_list[[cond]], function(x) x$fits))
  
  pval_list = unlist(lapply(cond_fit75_list[[cond]], function(x) x$pval))
  names(pval_list) = hvg
  
  pval_df = data.frame(pval_list)
  
  hep75_fits_qval[[cond]] = list("fits" = fits_df, "pval" = pval_df,
                                 "fdr" = apply(pval_df, 2, function(x) p.adjust(x, method = "fdr")))
}
end_time <- Sys.time()
end_time - start_time
saveRDS(hep75_fits_qval, file = "results/zonation_cond/hep_75_fits_qval.RDS")

## test between conditions
numCores = 10 #larger values resulted in problems
sub_dat = all_hep_cells[,all_hep_cells$zonation_pt<0.75]
start_time <- Sys.time()
pval_list = mclapply(hvg, processInput, mc.cores = numCores)
names(pval_list) = hvg
end_time <- Sys.time()
end_time - start_time

pval_df = data.frame(Reduce(rbind, pval_list))
pval_df = apply(pval_df, 2, function(x) unlist(x))
pval_nodon = pval_df[grepl("nodon", rownames(pval_df)),]
pval_wdon = pval_df[grepl("wdon", rownames(pval_df)),]
rownames(pval_nodon) = hvg
rownames(pval_wdon) = hvg

hep75_res_cond = list("pval_nodon" = pval_nodon,
                      "pval_wdon" = pval_wdon,
                      "fdr_nodon" = apply(pval_nodon, 2, function(x) p.adjust(x, method = "fdr")),
                      "fdr_wdon" = apply(pval_wdon, 2, function(x) p.adjust(x, method = "fdr")))
saveRDS(hep75_res_cond, file = "results/zonation_cond/hep_75_cond_tests.RDS")

## test between conditions - scaled
var_pval = cbind(hep75_fits_qval$healthy$fdr,
                 hep75_fits_qval$embolised$fdr,
                 hep75_fits_qval$regenerating$fdr)
varcond = apply(var_pval, 1, function(x) any(x<=0.05))
hep_sig_fits = lapply(hep75_fits_qval, function(x) x$fits[,varcond])
gvar = apply(data.frame(lapply(hep_sig_fits, function(x) apply(x, 2, function(y) diff(range(y))>=0.05))), 1,
             function(x) all(x))
sub_hvg = hvg[varcond & hvg %in% names(gvar)[gvar]]
numCores = 16
start_time <- Sys.time()
pval_scaled_list = mclapply(sub_hvg, function(x) deCondScale(hep_cells, x, 0.75), 
                            mc.cores = numCores)
names(pval_scaled_list) = sub_hvg
end_time <- Sys.time()
end_time - start_time
saveRDS(pval_scaled_list, file = "results/zonation_cond/hep75_pval_scaled_list.RDS")
```
...
#hep_g_diff_cond = apply(hep75_res_cond$fdr_nodon, 1, function(x) any(x<=0.05))

hep_sig_fits = lapply(hep75_fits_qval, function(x) x$fits[,varcond])
gvar = apply(data.frame(lapply(hep_sig_fits, function(x) apply(x, 2, function(y) diff(range(y))>=0.05))), 1,
             function(x) all(x))

hep_sc_fits = lapply(hep_sig_fits, function(x) t(apply(x[,gvar], 2, 
                     function(x) if(all(x==x[1])){rep(0, length(x))} else{scale(x)})))
lapply(hep_sc_fits, dim)


eg_all = clusterProfiler::bitr(as.character(rownames(hep75_fits_qval$healthy$fdr)), fromType="SYMBOL", 
                               toType="ENTREZID", OrgDb="org.Hs.eg.db")
eg_sig = clusterProfiler::bitr(as.character(rownames(hep_sc_fits$healthy)), fromType="SYMBOL", 
                               toType="ENTREZID", OrgDb="org.Hs.eg.db")
got_all = clusterProfiler::enrichGO(gene = eg_sig$ENTREZID, universe = eg_all$ENTREZID, 
                                    OrgDb = org.Hs.eg.db, ont = "BP", pvalueCutoff  = 0.01,
                                    qvalueCutoff  = 0.05, 
                                    pAdjustMethod = "BH", readable = T)@result

hcl = hclust(dist(Reduce(cbind, hep_sc_fits)), method = "ward.D")

cls = cutree(hcl, 8)

plot_df_l = list()
for(n in names(hep_sc_fits)){
  plot_df = reshape2::melt(t(hep_sc_fits[[n]]))
  plot_df = merge(plot_df, data.frame(cls), by.x = 2, by.y = 0)
  plot_df = merge(plot_df, data.frame(hep75_res_cond$fdr_nodon[,1]), by.x = 1, by.y = 0)
  plot_df$cond = n
  plot_df_l[[n]] = plot_df
}
plot_df = Reduce(rbind, plot_df_l)
plot_df$cond = factor(plot_df$cond, levels = unique(plot_df$cond))
ggplot(plot_df, aes(x = Var1, y = value, group = Var2))+
  facet_grid(cond~cls)+
  geom_line(alpha = 0.1, colour = "grey20")+
...
genes_cl = unique(plot_df[,c(1,4)])
#eg_all = clusterProfiler::bitr(as.character(genes_cl$Var2), fromType="SYMBOL", 
#                                 toType="ENTREZID", OrgDb="org.Hs.eg.db")
got_list = list()
got_cl_list = list()
for(i in unique(genes_cl$cls)){
  ...
  ncl = 3
  if(nrow(got_list[[as.character(i)]])>ncl){
    got_cl_list[[as.character(i)]] = getTopTerms(got_list[[as.character(i)]], ncl = ncl, nt = 2)
  } else{
    got_cl_list[[as.character(i)]] = got_list[[as.character(i)]]
  }
}
