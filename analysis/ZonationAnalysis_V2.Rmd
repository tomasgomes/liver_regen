...
processInput <- function(g) {
  d = data.frame(z=sub_dat@assays$SCT@data[g,], t=sub_dat$zonation_pt, 
                 cond=sub_dat$Condition, don = sub_dat$Donor)
  d$cond = as.character(d$cond)
  
  pval_list = list("all" = list("nodon" = c(), "wdon" = c()), "h_v_e" = list("nodon" = c(), "wdon" = c()),
                 "h_v_r" = list("nodon" = c(), "wdon" = c()), "e_v_r" = list("nodon" = c(), "wdon" = c()),
                 "hr_v_e" = list("nodon" = c(), "wdon" = c()), "h_v_er" = list("nodon" = c(), "wdon" = c()),
                 "he_v_r" = list("nodon" = c(), "wdon" = c()))
  
  for(comp in c("all", "h_v_e", "h_v_r", "e_v_r", "hr_v_e", "h_v_er", "he_v_r")){
    sub_d = d
    if(comp=="h_v_e"){
      sub_d = sub_d[sub_d$cond!="regenerating",]
    } else if(comp=="h_v_r"){
      sub_d = sub_d[sub_d$cond!="embolised",]
    } else if(comp=="e_v_r"){
      sub_d = sub_d[sub_d$cond!="healthy",]
      
    } else if(comp=="hr_v_e"){
      sub_d$cond[sub_d$cond!="embolised"] = "not_embolised"
    } else if(comp=="h_v_er"){
      sub_d$cond[sub_d$cond!="healthy"] = "not_healthy"
    } else if(comp=="he_v_r"){
      sub_d$cond[sub_d$cond!="regenerating"] = "not_regenerating"
    }
    
    null1 = suppressMessages(gam(z ~ ns(t, df = 3), data=sub_d))
    null2 = suppressMessages(gam(z ~ ns(t, df = 3) + don, data=sub_d))
    alt1 = suppressMessages(gam(z ~ ns(t, df = 3) * cond, data=sub_d))
    alt2 = suppressMessages(gam(z ~ ns(t, df = 3) * cond + don, data=sub_d))
    
    null1_anova = summary(null1)$parametric.anova
    null2_anova = summary(null2)$parametric.anova
    alt1_anova = summary(alt1)$parametric.anova
    alt2_anova = summary(alt2)$parametric.anova
    
    f1 = alt1_anova$"Mean Sq"[4] / null1_anova$"Mean Sq"[2]
  	p1 = pf(f1, alt1$df.residual, null1$df.residual, lower.tail=T)
  	f2 = alt2_anova$"Mean Sq"[5] / null2_anova$"Mean Sq"[3]
  	p2 = pf(f2, alt2$df.residual, null2$df.residual, lower.tail=T)
    
    pval_list[[comp]][["nodon"]] = c(pval_list[[comp]][["nodon"]], p1)
    pval_list[[comp]][["wdon"]] = c(pval_list[[comp]][["wdon"]], p2)
  }
  return(pval_list)
}
...
hep_sig_fits = lapply(hep75_fits_qval, function(x) x$fits[,hep_g_diff_cond])
...
hcl = hclust(dist(Reduce(cbind, hep_sc_fits)), method = "ward.D2")
...
for(n in names(hep_sc_fits)){
  ...
  plot_df = merge(plot_df, data.frame(hep_res_cond$fdr_nodon[,1]), by.x = 1, by.y = 0)
  plot_df$cond = n
  plot_df_l[[n]] = plot_df
}
...
for(i in unique(genes_cl$cls)){
  ...
  got_list[[as.character(i)]] = got@result
}

saveRDS(plot_df, file = "results/zonation_cond/hep_clustering_75_fits.RDS")
saveRDS(got_list, file = "results/zonation_cond/hep_75_fits_clustering_go.RDS")
...
hep_g_diff_cond = apply(hep_res_cond$fdr_nodon, 1, function(x) any(x<=0.05))

hep_sig_fits = lapply(hep_fits_list, function(x) x[,hep_g_diff_cond])
...
hep_sc_fits75 = lapply(hep_sig_fits, 
                       function(x) t(apply(x, 2, function(x) if(all(x==x[1])){rep(0, length(x))} else{scale(x)})))

cl_list = list()
gr_list = list()
for(n in names(hep_sc_fits)){
  cl_list[[paste0(n, "_all")]] = hclust(dist(hep_sc_fits[[n]]), method = "ward.D2")
  cl_list[[paste0(n, "_75")]] = hclust(dist(hep_sc_fits75[[n]]), method = "ward.D2")
  
  gr_list[[paste0(n, "_all")]] = cutree(cl_list[[paste0(n, "_all")]], 4)
  gr_list[[paste0(n, "_75")]] = cutree(cl_list[[paste0(n, "_75")]], 4)
}

comb_cond = combn(names(hep_sc_fits), 2)
for(i in 1:ncol(comb_cond)){
  n1 = comb_cond[1,i]
  n2 = comb_cond[2,i]
  cl_list[[paste0(n1,"-",n2,"_all")]] = hclust(dist(hep_sc_fits[[n1]]-hep_sc_fits[[n2]]), method = "ward.D2")
  cl_list[[paste0(n1,"-",n2,"_75")]] = hclust(dist(hep_sc_fits75[[n1]]-hep_sc_fits75[[n2]]), 
                                              method = "ward.D2")
  
  gr_list[[paste0(n1,"-",n2,"_all")]] = cutree(cl_list[[paste0(n1,"-",n2,"_all")]], 4)
  gr_list[[paste0(n1,"-",n2,"_75")]] = cutree(cl_list[[paste0(n1,"-",n2,"_75")]], 4)
}

hep_fits_all_list = c(hep_sc_fits, unname(hep_sc_fits75))
names(hep_fits_all_list) = c(names(cl_list)[grepl("all", names(cl_list))][1:3],
                             names(cl_list)[grepl("75", names(cl_list))][1:3])
for(i in 1:ncol(comb_cond)){
  n1 = comb_cond[1,i]
  n2 = comb_cond[2,i]
  hep_fits_all_list[[paste0(n1,"-",n2,"_all")]] = hep_sc_fits[[n1]]-hep_sc_fits[[n2]]
  hep_fits_all_list[[paste0(n1,"-",n2,"_75")]] = hep_sc_fits75[[n1]]-hep_sc_fits75[[n2]]
}

plot_df_l = list()
for(n in names(gr_list)){
  plot_df = reshape2::melt(t(hep_fits_all_list[[n]]))
  plot_df = merge(plot_df, data.frame(gr_list[[n]]), by.x = 2, by.y = 0)
  plot_df$cond = n
  colnames(plot_df)[4] = "cls"
  plot_df_l[[n]] = plot_df
  plt = ggplot(plot_df_l[[n]], aes(x = Var1, y = value, group = Var2))+
    facet_wrap(~cls)+
    geom_line(alpha = 0.1, colour = "grey20")+
    geom_hline(yintercept = 0, colour = "white")+
    labs(title = n)+
...
anncl = list("healthy_all" = c("cl1" = "periportal", "cl2" = "pericentral", "cl3" = "portal bump", "cl4" = "central bump"), 
             "healthy_75" = c("cl1" = "periportal", "cl2" = "pericentral", "cl3" = "non-midzone", "cl4" = "midzone"), 
             "embolised_all" = c("cl1" = "periportal", "cl2" = "portal bump", "cl3" = "central bump", "cl4" = "non-central"), 
             "embolised_75" = c("cl1" = "periportal", "cl2" = "pericentral", "cl3" = "pericentral", "cl4" = "non-midzone"), 
             "regenerating_all" = c("cl1" = "non-central", "cl2" = "midzone", "cl3" = "central bump", "cl4" = "portal bump"), 
             "regenerating_75" = c("cl1" = "periportal", "cl2" = "periportal", "cl3" = "pericentral", "cl4" = "non-midzone"),
             "healthy-embolised_all" = c("cl1" = "Eportal_wiggle_Hcentral", "cl2" = "Hcentral", "cl3" = "Eportal_Hcentral", "cl4" = "Hportal"), 
             "healthy-embolised_75" = c("cl1" = "constant", "cl2" = "Hmidzone", "cl3" = "Hportal", "cl4" = "Eportal"), 
             "healthy-regenerating_all" = c("cl1" = "constant", "cl2" = "Hcentral", "cl3" = "Rmidzone", "cl4" = "Rportal_Hcentral"), 
             "healthy-regenerating_75" = c("cl1" = "constant", "cl2" = "Rportal", "cl3" = "Hportal", "cl4" = "Hmidzone"),
             "embolised-regenerating_all" = c("cl1" = "constant", "cl2" = "Eportal_Rcentral", "cl3" = "Eportal_wiggle_Rcentral", "cl4" = "Emidzone"), 
             "embolised-regenerating_75" = c("cl1" = "constant", "cl2" = "Eportal", "cl3" = "Eportal", "cl4" = "Ecentral"))
anncl = data.frame(anncl, check.names = F)

gr_n_list = list()
for(n in names(gr_list)){
  gr_n_list[[n]] = anncl[,n][gr_list[[n]]]
  names(gr_n_list[[n]]) = names(gr_list[[n]])
}

saveRDS(list("all" = data.frame(gr_n_list[c(1,3,5)]), "75" = data.frame(gr_n_list[c(2,4,6)]), 
             "vs_all" = data.frame(gr_n_list[c(7,9,11)]), "vs_75" = data.frame(gr_n_list[c(8,10,12)])),
        file = "results/zonation_cond/hep_indiv_cluster_comp.RDS")
```
...
g = "HAMP"
par(mfrow = c(1,3))
t1 = all_hep_cells$zonation_pt[all_hep_cells$Condition=="healthy"]
exp1 = all_hep_cells@assays$SCT@data[g,all_hep_cells$Condition=="healthy"]
plot(t1, exp1, pch = 20)

t2 = all_hep_cells$zonation_pt[all_hep_cells$Condition=="embolised"]
exp2 = all_hep_cells@assays$SCT@data[g,all_hep_cells$Condition=="embolised"]
plot(t2, exp2, pch = 20)

t3 = all_hep_cells$zonation_pt[all_hep_cells$Condition=="regenerating"]
exp3 = all_hep_cells@assays$SCT@data[g,all_hep_cells$Condition=="regenerating"]
plot(t3, exp3, pch = 20)

par(mfrow = c(1,1))
minmax = c(min(c(hep_fits_list$healthy[,g],hep_fits_list$regenerating[,g],hep_fits_list$embolised[,g]))-0.1,
           max(c(hep_fits_list$healthy[,g],hep_fits_list$regenerating[,g],hep_fits_list$embolised[,g]))+0.1)
plot(hep_fits_list$healthy[,g], ylim = minmax)
points(hep_fits_list$regenerating[,g], col = "blue")
points(hep_fits_list$embolised[,g], col = "red")
```
...
hep_dtw_dists = lapply(hep_sc_fits, dtw::dtwDist, window.type = "itakura") # this takes many hours!!!
saveRDS(hep_dtw_dists, file = "results/zonation_cond/hep_dtw_dists_DE.RDS")

hcl = hclust(dist(hep_sc_fits$regenerating), method = "ward.D2")
hcl = hclust(dist(Reduce(cbind, hep_sc_fits)), method = "ward.D2")
...
View(unique(plot_df[,c(1,4,5)]))

eg_all = clusterProfiler::bitr(as.character(rownames(hep_res_cond$fdr_nodon)), 
                               fromType="SYMBOL", toType="ENTREZID", OrgDb="org.Hs.eg.db")
# convert gene IDs
eg = clusterProfiler::bitr(unique(plot_df$Var2[plot_df$cls==3]), fromType="SYMBOL", 
                           toType="ENTREZID", OrgDb="org.Hs.eg.db")
...
all_terms = as.data.frame(got@result)
...
library(parallel)
processInput <- function(g) {
  d = data.frame(z=all_hep_cells@assays$SCT@data[g,], t=all_hep_cells$zonation_pt, 
                 cond=all_hep_cells$Condition, don = all_hep_cells$Donor)
  d$cond = as.character(d$cond)
...
numCores = 8 #larger values resulted in problems
...
hep_res_cond = list("pval_nodon" = pval_nodon,
                    "pval_wdon" = pval_wdon,
                    "fdr_nodon" = apply(pval_nodon, 2, function(x) p.adjust(x, method = "fdr")),
                    "fdr_wdon" = apply(pval_wdon, 2, function(x) p.adjust(x, method = "fdr")))

saveRDS(hep_res_cond, file = "results/zonation_cond/hep_res_cond.RDS")
```
...
for(cond in names(cond_fit75_list)){
  ...
  end75_fits_qval[[cond]] = list("fits" = fits_df, "pval" = pval_df,
                                 "fdr" = apply(pval_df, 2, function(x) p.adjust(x, method = "fdr")))
}
end_time <- Sys.time()
end_time - start_time
...
end75_res_cond = list("pval_nodon" = pval_nodon,
                      "pval_wdon" = pval_wdon,
                      "fdr_nodon" = apply(pval_nodon, 2, function(x) p.adjust(x, method = "fdr")),
                      "fdr_wdon" = apply(pval_wdon, 2, function(x) p.adjust(x, method = "fdr")))
...
end_g_diff_cond = apply(end75_res_cond$fdr_nodon, 1, function(x) any(x<=0.05))

end_sig_fits = lapply(end75_fits_qval, function(x) x$fits[,end_g_diff_cond])
gvar = apply(data.frame(lapply(end_sig_fits, function(x) apply(x, 2, function(y) diff(range(y))>=0.05))), 1,
             function(x) all(x))

end_sc_fits = lapply(end_sig_fits, function(x) t(apply(x[,gvar], 2, 
                     function(x) if(all(x==x[1])){rep(0, length(x))} else{scale(x)})))
lapply(end_sc_fits, dim)

hcl = hclust(dist(Reduce(cbind, end_sc_fits)), method = "ward.D2")


cls = cutree(hcl, 4)

plot_df_l = list()
for(n in names(end_sc_fits)){
  plot_df = reshape2::melt(t(end_sc_fits[[n]]))
  plot_df = merge(plot_df, data.frame(cls), by.x = 2, by.y = 0)
  plot_df = merge(plot_df, data.frame(end_res_cond$fdr_nodon[,1]), by.x = 1, by.y = 0)
  plot_df$cond = n
  plot_df_l[[n]] = plot_df
}
plot_df = Reduce(rbind, plot_df_l)
...
saveRDS(plot_df, file = "results/zonation_cond/end_clustering_all_fits.RDS")
saveRDS(end75_res_cond, file = "results/zonation_cond/end_all_cond_tests.RDS")
saveRDS(end75_fits_qval, file = "results/zonation_cond/end_all_fits_qval.RDS")
...
processInput <- function(g) {
  d = data.frame(z=all_end_cells@assays$SCT@data[g,], t=all_end_cells$zonation_pt, 
                 cond=all_end_cells$Condition, don = all_end_cells$Donor)
  d$cond = as.character(d$cond)
...
end_res_cond = list("pval_nodon" = pval_nodon,
                    "pval_wdon" = pval_wdon,
                    "fdr_nodon" = apply(pval_nodon, 2, function(x) p.adjust(x, method = "fdr")),
                    "fdr_wdon" = apply(pval_wdon, 2, function(x) p.adjust(x, method = "fdr")))

xxx = apply(end_res_cond$fdr_nodon, 1, function(x) any(x<=0.05))
sum(xxx)
```
...
g = "SAA2"
par(mfrow = c(1,3))
t1 = all_end_cells$zonation_pt[all_end_cells$Condition=="healthy"]
exp1 = all_end_cells@assays$SCT@data[g,all_end_cells$Condition=="healthy"]
plot(t1, exp1, pch = 20)

t2 = all_end_cells$zonation_pt[all_end_cells$Condition=="embolised"]
exp2 = all_end_cells@assays$SCT@data[g,all_end_cells$Condition=="embolised"]
plot(t2, exp2, pch = 20)

t3 = all_end_cells$zonation_pt[all_end_cells$Condition=="regenerating"]
exp3 = all_end_cells@assays$SCT@data[g,all_end_cells$Condition=="regenerating"]
plot(t3, exp3, pch = 20)

par(mfrow = c(1,1))
minmax = c(min(c(end_fits_list$healthy[,g],end_fits_list$regenerating[,g],end_fits_list$embolised[,g]))-0.1,
           max(c(end_fits_list$healthy[,g],end_fits_list$regenerating[,g],end_fits_list$embolised[,g]))+0.1)
plot(end_fits_list$healthy[,g], ylim = minmax)
points(end_fits_list$regenerating[,g], col = "blue")
points(end_fits_list$embolised[,g], col = "red")
```

```{r}
end_g_diff_cond = apply(end_res_cond$fdr_nodon, 1, function(x) any(x<=0.05))

end_sig_fits = lapply(end_fits_list, function(x) x[,end_g_diff_cond])

end_sc_fits = lapply(end_sig_fits, function(x) t(apply(x, 2, 
                     function(x) if(all(x==x[1])){rep(0, length(x))} else{scale(x)})))

end_dtw_dists = lapply(end_sc_fits, dtw::dtwDist, window.type = "itakura")
saveRDS(end_dtw_dists, file = "results/zonation_cond/end_dtw_dists_DE.RDS")

hcl = hclust(as.dist(end_dtw_dists$healthy), method = "ward.D")

hcl = hclust(dist(end_sc_fits$regenerating), method = "ward.D2")
hcl = hclust(dist(Reduce(cbind, end_sc_fits)), method = "ward.D2")
...
for(i in 1:length(hvg)){
  ...
  d = data.frame(z=all_end_cells@assays$SCT@data[g,], t=all_end_cells$zonation_pt, 
                 cond=all_end_cells$Condition, don = all_end_cells$Donor)
  null1 = suppressMessages(gam(z ~ ns(t, df = 4), data=d))
  null2 = suppressMessages(gam(z ~ ns(t, df = 4) + don, data=d))
  alt1 = suppressMessages(gam(z ~ ns(t, df = 4) * cond, data=d))
  alt2 = suppressMessages(gam(z ~ ns(t, df = 4) * cond + don, data=d))
...
fdr1 = p.adjust(p1_v, method = "fdr")
fdr2 = p.adjust(p2_v, method = "fdr")

df_sigs = data.frame(hvg, p1_v, fdr1, p2_v, fdr2)
```
...
meta_df = data.frame("pt" = all_end_cells$zonation_pt,
                     "donors" = all_end_cells$Donor,
                     "cond" = all_end_cells$Condition)
meta_df$bins = cut(meta_df$pt, 100)
...
for(i in 1:length(hvg)){
  g = hvg[i]
  z = all_end_cells@assays$SCT@data[g,]
  
  d = aggregate(z~bins+donors+cond, data = cbind(meta_df, z), FUN="mean")
  cc = data.frame(table(meta_df$bins, 
                        as.factor(paste0(as.character(meta_df$cond), "_", as.character(meta_df$donors)))))
  cc = cc[cc$Freq==0,]
  cc$cond = unlist(lapply(strsplit(as.character(cc$Var2), "_"), function(x) x[1]))
  cc$donors = unlist(lapply(strsplit(as.character(cc$Var2), "_"), function(x) x[2]))
  cc = cc[,c(1,5,4,3)]
  colnames(cc) = colnames(d)
  d = rbind(d, cc)
  d = d[order(d$bins, d$cond, d$donors, decreasing = F),]
  d$t = rep(t, each = length(unique(paste0(d$donors, d$cond))))
  
  null1 = suppressMessages(gam(z ~ ns(t, df = 3), data=d))
  null2 = suppressMessages(gam(z ~ ns(t, df = 3) + donors, data=d))
  alt1 = suppressMessages(gam(z ~ ns(t, df = 3) * cond, data=d))
  alt2 = suppressMessages(gam(z ~ ns(t, df = 3) * cond + donors, data=d))
...
fdr1_b = p.adjust(p1_v_b, method = "fdr")
fdr2_b = p.adjust(p2_v_b, method = "fdr")

df_sigs_b = data.frame(hvg, p1_v_b, fdr1_b, p2_v_b, fdr2_b)
```
...
saveRDS(hep_mean_bins, file = "results/zonation_cond/zonation_bins_hep_list_rank.RDS")
...
saveRDS(end_mean_bins, file = "results/zonation_cond/zonation_bins_end_list_rank.RDS")
```
...
for(i in 1:ncol(comb_cond)){
  ...
  gene_groups_list[[comb_cond[1,i]]] = as.character(df_cor$gene[df_cor[[comb_cond[1,i]]]])
  gene_groups_list[[comb_cond[2,i]]] = as.character(df_cor$gene[df_cor[[comb_cond[2,i]]]])
  gene_groups_list[[paste0(comb_cond[1,i], "_all")]] = c(gene_groups_list$common,
                                                         as.character(df_cor$gene[df_cor[[comb_cond[1,i]]]]))
  gene_groups_list[[paste0(comb_cond[2,i], "_all")]] = c(gene_groups_list$common,
                                                         as.character(df_cor$gene[df_cor[[comb_cond[2,i]]]]))
