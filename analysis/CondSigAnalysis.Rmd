...
```{r}
donor_filt = donor_submk_list$all[donor_submk_list$all$p_val_adj<=0.05 &
                                    donor_submk_list$all$avg_logFC>=0.2,]
xxx = table(donor_filt$gene, donor_filt$cluster)
ndon_genes = rowSums(xxx)
ndonH_genes = rowSums(xxx[,grepl("HD", colnames(xxx))])
ndonD_genes = rowSums(xxx[,grepl("DD", colnames(xxx))])
conddiff = ndonH_genes>ndonD_genes | ndonH_genes<ndonD_genes

g_c_l = list()
dat = allcells_css@assays$SCT@data[,!is.na(allcells_css@meta.data$cell_type)]
for(n in 1:nrow(allcells_css@assays$SCT@data)){
  g_c_l[[n]] = tapply(dat[n,], allcells_css$Donor[!is.na(allcells_css@meta.data$cell_type)], 
                      function(y) sum(y>0))
}
```
...
meta[grepl("LSEC_", meta$cell_type),"cell_type"] = as.character(allcells_css@meta.data[rownames(meta)[grepl("LSEC_", meta$cell_type)],"allcells_simp"])
meta[grepl("Hepatocytes_", meta$cell_type), "cell_type"] = "Hepatocytes"
```
