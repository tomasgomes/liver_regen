...
```{r}
net_names_l = readRDS(file = "results/cell_comm/v3/count_net_list.RDS")
# list per condition with interactions, genes, mean
reform_list = readRDS(file = "results/cell_comm/v3/reform_list.RDS")
# list per cond with ligand or receptor expression for each interaction
scoring_mats = readRDS(file = "results/cell_comm/v3/scoring_mats.RDS")
...
ct_g_cond_ann = readRDS(file = "results/cell_comm/v3/ct_g_cond_ann.RDS")
# interaction with L and R, pair of cell types, mean exp, and function
ct_int_exp_l = readRDS(file = "results/cell_comm/v3/interact_celltype_exp_group_list.RDS")
...
# mut info data
her_allint = readRDS(file = "./results/cell_comm/v3/interactions_mutInfo_condComp.RDS")
# GSEA on mutual info results
all_gsea_list = readRDS(file = "./results/cell_comm/v3/allgsea_interactions_mutInfo_condComp.RDS")
simp_gsea_list = readRDS(file = "./results/cell_comm/v3/simpgsea_interactions_mutInfo_condComp.RDS")
# network data
load("results/cell_comm/v3/median_networks_cond.RData")
load("results/cell_comm/v3/networks_cond.RData")
load("results/cell_comm/v3/median_networks_cond_midct.RData")
load("results/cell_comm/v3/median_networks_cond_umap.RData")
load("results/cell_comm/v3/networks_cond_umap.RData")
load("results/cell_comm/v3/median_networks_cond_umap_midct.RData")
comph_inters = readRDS(file = "results/cell_comm/v3/comph_inters.RDS")
```
...
plt_counts = ggplot(net_names_all, aes(x = Var1, y = value))+
  ...
  scale_y_continuous(expand = c(0,0), limits = c(0,7300))+
...
perc_df = data.frame("Var1" = net_names_all$Var1,
                     ...
                     "perc" = c(rep(0, length(unique(net_names_all$Var1))),
                                (net_names_all$value[net_names_all$Var2=="embolised"]-net_names_all$value[net_names_all$Var2=="healthy"])/total_unique[as.character(net_names_all$Var1)[net_names_all$Var2=="embolised"],"value"],
                                (net_names_all$value[net_names_all$Var2=="regenerating"]-net_names_all$value[net_names_all$Var2=="healthy"])/total_unique[as.character(net_names_all$Var1)[net_names_all$Var2=="regenerating"],"value"])*100)
...
perc_plt_emb = ggplot(perc_df[perc_df$Var2=="embolised",], aes(x = Var1, y = perc, fill = col))+
  ...
  scale_y_continuous(expand = c(0,0), limits = c(-35,55))+
  coord_flip()+
...
pdf("figure_panels_rev2/fig_comm/mainFig_countsCond.pdf", height = 7, width = 7)
cowplot::plot_grid(cowplot::plot_grid(plt_counts+theme(legend.position = "none"), 
...
pdf("figure_panels_rev2/fig_comm/mainFig_funcPart.pdf", height = 5.5, width = 4.5)
print(bar_func_sub)
dev.off()
...
pdf("figure_panels_rev2/fig_comm/suppFig_countsFunc.pdf", paper = "a4", width = 9, height = 13.5)
print(bar_func_all)
dev.off()
```
...
pdf("figure_panels_rev2/fig_comm/mainFig_funcGSEA.pdf", height = 2.2, width = 2.85)
print(gseaplt)
dev.off()
```
...
write.csv(her_allint_f[,c(1:8,28)], file = "figure_panels_rev2/fig_comm/all_interactions_functions.csv",
          col.names = T, row.names = F, quote = F)
# remove autocrine
...
pdf("figure_panels_rev2/fig_comm/mainFig_ecmInteract.pdf", height = 3, width = 7.5)
print(ecm_plt)
dev.off()
...
pdf("figure_panels_rev2/fig_comm/mainFig_devInteract.pdf", height = 3, width = 7.5)
print(dev_plt)
dev.off()
```
...
pltboth = ggplot()+
  ...
  theme(aspect.ratio = 1, 
        ...
        legend.position = "none")
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_all.pdf", 
...
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_median.pdf", height = 3.2, width = 5, useDingbats = F)
...
pltboth = ggplot()+
  ...
  theme(aspect.ratio = 1, 
        ...
        legend.spacing.y = unit(0.1, "cm"),
        legend.position = "none")
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_mid.pdf", 
    height = 3.1, width = 5.2, useDingbats = F)
print(pltboth)
dev.off()
...
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_median_mid.pdf", 
...
pltboth = ggplot()+
  ...
  theme(aspect.ratio = 1, 
        ...
        legend.spacing.y = unit(0.1, "cm"),
        legend.position = "none")
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_umap_all.pdf", height = 3.1, width = 5.2, useDingbats = F)
print(pltboth)
dev.off()
...
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_umap_median.pdf", height = 3.2, width = 5, useDingbats = F)
print(plt_cond_l_ch$healthy)
print(plt_cond_l_ch$embolised)
print(plt_cond_l_ch$regenerating)
dev.off()
...
pltboth = ggplot()+
  ...
  theme(aspect.ratio = 1, 
        ...
        legend.spacing.y = unit(0.1, "cm"),
        legend.position = "none")
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_umap_all_mid.pdf", height = 3.1, width = 5.2, useDingbats = F)
print(pltboth)
dev.off()
...
pdf("figure_panels_rev2/fig_comm/mainFig_inet_cond_umap_median_mid.pdf", height = 3.2, width = 5, useDingbats = F)
print(plt_cond_l_ch$healthy)
print(plt_cond_l_ch$embolised)
print(plt_cond_l_ch$regenerating)
dev.off()
...
pdf("figure_panels_rev2/fig_comm/mainFig_immuneNetInteract_many.pdf", height = 3, width = 50)
print(imm_plt)
dev.off()
...
plot_df = plot_df[!(plot_df$intlr %in% c(devdf$intlr, ecmdf$intlr)),]
imm_plt = pltFunc(plot_df)
pdf("figure_panels_rev2/fig_comm/mainFig_immuneNetInteract.pdf", height = 2.86, width = 25)
print(imm_plt)
dev.off()
```
...
pdf("figure_panels_rev2/fig_comm/suppFig_countsCond.pdf", height = 18, width = 18)
...
pdf("figure_panels_rev2/fig_comm/suppFig_countsDiff.pdf", height = 10, width = 20)
...
for(n in names(inter_df)[1:3]){
  ...
  pltcnts = ggplot(plot_df[order(plot_df$Freq, decreasing = F),], 
    ...
    scale_colour_gradientn(colors = c("#ededed", "#deebf7", "#9ecae1", "#bcbddc", "#756bb1"), 
                           limits = c(0, 500))+
    scale_size_continuous(limits = c(0, 500))+
    theme_classic()+
    theme(aspect.ratio = 1,
          panel.background = element_rect(colour = "black"),
          axis.line = element_blank(),
...
pdf("figure_panels_rev2/fig_comm/suppFig_countsCond_mid.pdf", height = 6.9, width = 6.9)
cnts_list[[1]]+cnts_list[[3]]+cnts_list[[2]]+cnts_list[[4]]+      patchwork::plot_layout(ncol = 2)
dev.off()
```
...
pdf("figure_panels_rev2/fig_comm/suppFig_countsDiff_mid.pdf", height = 3.45, width = 6.9)
...
pdf("figure_panels_rev2/fig_comm/mainFig_InteractUnique.pdf", 
    height = 20, width = 2.8)
print(imm_plt)
dev.off()
...
plot_df = filtFuncU(her_allint_f[her_allint_f$id_cp_interaction %in% ctmid_inter_l$L2[!grepl("stress", ctmid_inter_l$value.y)],],
                   ...
                   expthr = 0.2, minct = 3)
imm_plt = pltFuncU(plot_df)
pdf("figure_panels_rev2/fig_comm/mainFig_InteractUnique_mid.pdf", 
    height = 32, width = 2.8)
print(imm_plt)
dev.off()
```
...
folder = "figure_panels_rev2/fig_comm/heatmaps_interact_indiv/"
...
folder = "figure_panels_rev2/fig_comm/heatmaps_interact_indiv_hpve/"
for(i in 1:length(unique(plot_df$intlr))){
  iii = unique(plot_df$intlr)[i]
  sub_plot_df = plot_df[plot_df$intlr==iii,]
...
perc_plt_emb = ggplot(perc_df[perc_df$Var2=="embolised",], aes(x = Var1, y = perc, fill = col))+
  geom_col()+
  scale_fill_manual(values = c("deepskyblue1", "red1"))+
  scale_y_continuous(expand = c(0,0), limits = c(-20,35))+
  coord_flip()+
  labs(y = "% change\nembolised vs healthy", x = "Cell Type")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 30, vjust = 1, hjust = 1, colour = "black"),
...
pdf("figure_panels_rev2/fig_comm/mainFig_countsCond_mid.pdf", height = 3.35, width = 5.5)
cowplot::plot_grid(cowplot::plot_grid(plt_counts+theme(legend.position = "none"), 
                                      perc_plt_reg, perc_plt_emb, 
                                      ncol = 3, align = "h", axis = "r", rel_widths = c(1,0.4,0.4)),
                   leg, nrow = 2, rel_heights = c(1, 0.1))
...
pdf("figure_panels_rev2/fig_cir/endo_sig_vio.pdf", height = 2.8, width = 11)
print(plt)
dev.off()
```
