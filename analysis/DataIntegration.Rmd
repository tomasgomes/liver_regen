...
Subclustering the Kupffer-T cell interaction
...
# save markers in the same folder, renamed
mk_healthy_filt = mk_healthy_cl[mk_healthy_cl$p_val_adj<=0.05,]
meta_unique = unique(newmeta[,c(1,2)])
rownames(meta_unique) = meta_unique$clusters
mk_healthy_filt$cluster = meta_unique[mk_healthy_filt$cluster,"names_clusters"]
write.csv(x = mk_healthy_filt, col.names = T, row.names = T, quote = F,
          file = "results/integr_healthy/css_markers_plots/css_allPops_markers.csv")
```
...
```{r}
allcells_css = readRDS("data/processed/scripts_out/CSSIntegr_allcells.RDS")
#allcells_rpca = RunPCA(allcells_rpca, verbose = F, npcs = 80)
# load healthy data, which now contains annotations and can help here
...
ggplot(merge_umap, aes(x = UMACSS_1, y = UMACSS_2, colour = names_clusters))+
  geom_point(size = 0.25)+
...
ggplot(merge_umap, aes(x = UMACSS_1, y = UMACSS_2, colour = names_major))+
  geom_point(size = 0.25)+
  guides(colour = guide_legend(override.aes = list(size = 4), ncol = 1))+
  theme_classic()+
  theme(legend.position = "right")
...
for(i in c(0.6, 1)){
  n = paste0("res_", i)
  print(n)
  hcells_css = SetIdent(allcells_css, value = paste0("cssAll_res.", i))
