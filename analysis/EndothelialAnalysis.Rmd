...
# clustering
hc_rows = hclust(dist(t(scale(t(avg_endo_cond[unique(avg_endoc_meta$value),])))), method = "ward.D")
hc_cols = hclust(dist(scale(t(avg_endo_cond[unique(avg_endoc_meta$value),]))), method = "ward.D")
...
pheatmap::pheatmap(f_angio[order(rowSums(f_angio), decreasing = T),order(colSums(f_angio), decreasing = T)], clustering_method = "ward.D2", cluster_cols = F, cluster_rows = F)
```



```{r}
plot_df = reshape2::melt(f_angio[order(rowSums(f_angio), decreasing = F),
                                 order(colSums(f_angio), decreasing = T)])
